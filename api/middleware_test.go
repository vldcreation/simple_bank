...
func addAuth(t *testing.T, request *http.Request, tokenMaker token.Maker, authorizationType string, username string) {
	token, err := tokenMaker.CreateToken(username, time.Minute)
	require.NoError(t, err)
	authHeader := authorizationType + " " + token
	request.Header.Set(AuthHeader, authHeader)
}
...
func TestAuthMiddleware(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "OK",
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuth(t, request, tokenMaker, BearerSchema, "user")
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
		...
		},
		{
			...
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuth(t, request, tokenMaker, "unsupported", "user")
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusUnauthorized, recorder.Code)
			},
		},
		{
			...
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuth(t, request, tokenMaker, "", "user")
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusUnauthorized, recorder.Code)
			},
